...
for file in listdir(images_cells_dir):
    ...
    if not exists(dest) and id[1:] in mask_ids:
        if original:
            ...
            im.save(dest, quality = 100, subsampling=0) #irreversible transformation 
...
for file in listdir(masks_cells_dir):
    ...
    if not exists(dest):
        if original:
            ...
            x = np.asarray(im)
            normalized = (x.astype(np.uint16) - x.min()) * 255.0 / (x.max() - x.min())
            im = Image.fromarray(normalized.astype(np.uint8))
            im.save(dest, quality = 100, subsampling=0)
